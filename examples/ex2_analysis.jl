...
nn.plot_output(validation_data, freerun, θ_est[4, :],
...
boxplot!(p, ["SP"], [df[:series_parallel]])
boxplot!(p, ["P"], [df[:parallel]],  yscale=:log10)
...
x =1:100
#plot(1:100, sort(df[:series_parallel]), label= "SP" , yscale=:log10)
plot(1:100, sort(df[:parallel]), label= "P" , yscale=:log10)
